...
def render_trajectory(
    ...
    *,
    ...
    if camera is not None:
        ...
        # Camera tuned per arm — Feetech is ~35 cm, UR5e is ~85 cm reach.
        arm = env.scene.resolved().arm
        if arm == "feetech":
            cam.distance = 0.7
            cam.azimuth = 135.0
            cam.elevation = -25.0
    ...
    resolved = env.scene.resolved()
    n_arm_dofs = len(resolved.home_qpos)
    mujoco.mj_resetData(m, d)
    d.qpos[:n_arm_dofs] = resolved.home_qpos
